...
impl std::fmt::Debug for FunctionFilter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("FunctionFilter").field(&"<function>").finish()
    }
}
...
impl MqttClientManager {
    ...
    pub async fn subscribe(
        ...
        if !subs.contains_key(&topic) {
            self.client.subscribe(&topic, QoS::AtMostOnce).await.with_subscription_context(&topic)?;
    ...
    pub fn function_filter(f: impl Fn(&str) -> bool + Send + Sync + 'static) -> Box<dyn MessageFilter> {
